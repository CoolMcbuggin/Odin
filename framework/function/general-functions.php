...
function cmsmasters_get_tweets( $username, $count ) {
	$backup_name = 'cmsmasters_' . 'sports-store' . '_tweets_list_backup';

	$backup_tweets = get_option( $backup_name );

	if ( 'done' === get_transient( 'cmsmasters_' . 'sports-store' . '_tweets_list_regeneration' ) && ! empty( $backup_tweets ) ) {
		return $backup_tweets;
	}

	$cmsmasters_option = sports_store_get_global_options();

	if ( empty( $cmsmasters_option['sports-store' . '_twitter_access_token'] ) ) {
		return $backup_tweets;
	}
	...
	$response = wp_remote_get(
		...
		array(
			'headers' => array(
				'Authorization' => 'Bearer ' . esc_html( $cmsmasters_option['sports-store' . '_twitter_access_token'] ),
			),
			'body' => array(
	...
	set_transient( 'cmsmasters_' . 'sports-store' . '_tweets_list_regeneration', 'done', HOUR_IN_SECONDS * 6 );
